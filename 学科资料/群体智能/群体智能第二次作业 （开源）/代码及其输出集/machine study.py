...
parser.add_argument("--epochs", type=int, default=3000)  # 迭代多少局数
...
# 修改后的代码
model = torch.load("2048.pt", map_location=device, weights_only=True)
model.eval()
...
while True:
    ...
    plt.savefig(f'outputs/frame_{int(time.time() * 1000)}.png', bbox_inches='tight', pad_inches=0)
